...
class ZhihuSpider(Spider):
    handle_httpstatus_list = [500, 301, 302, 303, 304, ]
    name = "zhihu"
    ...
    def start_requests(self):
        yield Request(self.user_url.format(user=self.start_user, include=self.user_query), self.parse_user, meta={'proxy': 'http://93.179.66.57:8080/random', 'start': 'True'}, dont_filter=True)
        yield Request(self.follows_url.format(user=self.start_user, include=self.follows_query, limit=20, offset=0),
                      self.parse_follows, meta={'proxy': 'http://189.17.22.188:20183/random', 'start': 'True'}, dont_filter=True)
        yield Request(self.followers_url.format(user=self.start_user, include=self.followers_query, limit=20, offset=0),
                      self.parse_followers, meta={'proxy': 'http://117.202.20.70:555/random', 'start': 'True'}, dont_filter=True)
